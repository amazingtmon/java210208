...
public class Client_TalkClient extends JFrame implements ActionListener{
	...
	public static void main(String args[]) {
		...
		//메인 스레드 우선권
		Client_TalkClient tc = new Client_TalkClient();
		
		//화면 콜
		tc.initDisplay();
		
		//소켓 생성 -> TS쪽 serversocket에서 감지 -> 일반 소켓으로 전달
		//run메소드 안에서 client server thread생성 -> 생성자 호출(this) -> 듣기 가능해짐.
		//듣기가 가능해지기 위한 전제조건 : oos(홀수), ois(짝수), 소켓 객체가 있어야 가능하다.
		tc.init();
	}
