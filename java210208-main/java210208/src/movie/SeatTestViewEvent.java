	...
	public void itemStateChanged(ItemEvent ie) {
		...
		if(obj == JCheckBox.class) {
			...
			//상태변화가 좌석 선택 해제이면
				...
				stv.getJlb_selseatnum().setText(String.valueOf(--stv.inwon));
				//선택 해제한 좌석번호 가져오기
				String deseljlb = ((JCheckBox) ie.getSource()).getText();
				//선택 해제한 좌석번호 표시지우기
				for(int i=0; i<stv.getJlb_selseat().length;i++) {
					if(deseljlb.equals(stv.getJlb_selseat()[i].getText())) {
