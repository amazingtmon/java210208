...
public class Board41Logic {
	...
	public int boardInsert(Map<String, Object> pmap) {
		...
		int bm_no = 0;
		int bm_group = 0;
		if(pmap.get("bm_group") != null) {//read.jsp 에서 댓글쓰기 누름.
			bm_group = Integer.parseInt(pmap.get("bm_group").toString());
		}
		//댓글일때.
		if(bm_group > 0) {
			boardMDao.bmStepUpdate(pmap);//조건에 맞지 않으면 처리가 생략될 수 있다.
			pmap.put("bm_pos",Integer.parseInt(pmap.get("bm_group").toString())+1);
			pmap.put("bm_pos",Integer.parseInt(pmap.get("bm_step").toString())+1);
		}
		//새글일때
		else {
